...
public class
Change_Account extends AsyncTask<ChangeAccountData,Void,String>{

    //リスナー
    private Listener listener;
    private String readline;
    @Override
    protected String doInBackground(ChangeAccountData... accounts) {

        //受け取ったParamを格納
        ChangeAccountData account = accounts[0];
        //読み込み先の領域
        StringBuffer sb = new StringBuffer();
        //接続するためのクラスを宣言
        HttpURLConnection con = null;
        String new_account_url = "http://"+ AWS_INTERFACE.IPADDRESS +"/tanuki/changeAccount";
        ...
        try {
            ...
            malt_part_post.textPost(printStream,"UserNo",account.userNo,boundary);
            malt_part_post.textPost(printStream,"name",account.name,boundary);
            malt_part_post.textPost(printStream,"age",account.age,boundary);
            malt_part_post.textPost(printStream,"type",account.mytype,boundary);
            malt_part_post.textPost(printStream,"jiko",account.jiko,boundary);
